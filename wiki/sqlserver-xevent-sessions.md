...
created: 2026-08-15
updated: 2026-08-15
notion_page_id: "3befb969-b8be-81a2-9794-cbed2e65bc5a"
notion_synced: "2026-08-16T20:20:18+09:00"
---
...
> [!note] 왜 5초인가 — 원래 표준은 1초였다
>
> 블로킹 모니터는 **임계값 주기마다 같은 블로킹을 반복 보고한다.** 한 번 보고하고 마는 것이
> 아니다. 임계 1초면 5분 지속된 블로킹 하나가 보고서 수백 건이 되고, 파일이 순식간에
> 롤오버되어 **정작 필요한 과거 기록이 밀려난다.**^[inferred]
>
> 원본 구축 메모와 [[sqlserver-operations]]의 초기 기재값은 **1초**였고,
> **2026-08-15에 5초로 통일**했다. 두 페이지 모두 5초다 — 한쪽만 고치면 다시 갈라진다.
...
- **임계값 3초(느린 쿼리)·5초(블로킹)는 실측 근거가 없다.** 배포 후 하루치 파일 증가량을 보고
  조정해야 하며, 조정 근거를 이 페이지에 되먹여야 한다. 5초는 "1초보다 낫다"는 판단일 뿐
  최적값이 아니다
