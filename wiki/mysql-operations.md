...
> - **규모별 백업 도구 분기 기준이 정해져 있다**: 수 GB 이하 mysqldump / 수십 GB+ MySQL Shell / 초대형+짧은 RTO는 물리백업. 관리형 DB(RDS/Aurora) 대상이면 옵션 5종(`--set-gtid-purged=OFF`, `--no-tablespaces`, `strip_definers`, `strip_restricted_grants`, 타깃 `local_infile=1`)이 환경 무관 고정값이다 — 둘 다 자동화 가능하며 실행 상세는 [[mysql-dump-load]]
