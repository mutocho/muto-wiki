...
category: db운영
tags: [knowledge-management, security, backlog, notion]
summary: 전체 뎁스 수집(~140페이지)에서 발견된 교정 대상 20건 — 위험 명령 무경고 게재, 민감정보 잔존, 빈 페이지, 색인 드리프트. 우선순위 순.
...
> [!tip] 핵심 Takeaway
> - **P1 6건은 지식 정리 과제가 아니라 보안 사고 대기 상태다.** 실 서비스 스키마명이 박힌 TRUNCATE 생성기, QA RDS 실호스트명, 개인 Gmail 주소 — 미착수 상태이므로 우선 처리한다
> - **실행 불가하거나 오독을 유발하는 쿼리가 사내 문서에 살아있다** (MySQL에 PG 전용 `FILTER (WHERE)`, `blocked > 50`을 임계치로 오독). [[operational-queries]]에 옮길 때 그대로 베끼지 않는다
> - **색인 드리프트가 실측으로 확인됐다** (실제 33건 vs 기록 31건, ID 32 결번). 수기 집계는 반드시 어긋난다 — 자동 집계로 바꿔야 할 근거
> - 이 백로그가 [[db-security-review-patterns]] 체크리스트의 실증 데이터다. 패턴이 이론이 아니라 실제로 나온다는 증거
