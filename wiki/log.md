...
- [2026-08-16T20:20:23+09:00] SYNC_NOTION pages=35 created=0 updated=35 skipped=0 held=0 failed=0 target="DBA"
- [2026-08-16T20:18:59+09:00] SYNC_NOTION pages=31 created=0 updated=31 skipped=0 held=4 failed=0 target="DBA"
- [2026-08-16T20:17:01+09:00] SYNC_NOTION pages=30 created=0 updated=30 skipped=0 held=5 failed=0 target="DBA"
- [2026-08-16T20:16:16+09:00] SYNC_NOTION pages=0 created=0 updated=0 skipped=0 held=1 failed=0 target="DBA"
- [2026-08-16T20:15:41+09:00] SYNC_NOTION pages=35 created=34 updated=1 skipped=0 held=0 failed=0 target="DBA"
- [2026-08-16T20:12:10+09:00] SYNC_NOTION pages=1 created=1 updated=0 skipped=0 held=0 failed=0 target="DBA"
- [2026-08-16T20:10:00+09:00] FIX pages=5 note="3차 건강검진 수정(승인 범위: 경미한 불일치 2건·스키마 필드 등재·mysql-operations 승격·todo 정리. **Notion 재구축은 사용자가 보류**). ① operational-queries summary '11종'인데 열거는 10개 — 4번 `커넥션 현황` 누락분 추가. ② index.md 미완 과제 '분리된 3개 페이지 Notion 미동기화' → '`notion_page_id: null` 3개 페이지'로 재정의하고 postgresql-object-operations 편입. **추적 단위가 '분할'이라 08-16 신규 적재분이 통째로 빠져 있었다** — 단위를 사건(분할)이 아니라 상태(null)로 바꿔야 다음 신규 적재도 자동으로 걸린다. ③ CLAUDE.md §2.3에 `aliases`·`source_url`·`superseded_by`를 선택 필드로 등재하고 '이 표가 허용 필드의 전부'를 명문화 — 앞 두 개는 3회차 연속 위반으로 재보고됐으나 실사용 중이라 **필드가 아니라 표를 고쳤고**, `superseded_by`는 §2.5가 요구하면서 표에 없던 것이 그 과정에서 드러났다. ④ mysql-operations draft→reviewed(§2.5: 공식 릴리스 노트 대조 완료, 본문에 '실행 검증 전' 문구 없음). 나머지 승격 후보 4건은 본문·index가 '실행 검증 전'을 명시해 status만 올리면 충돌하므로 draft 유지. ⑤ todo.md 경과 일정 3건(치과·DBGW 리뷰·데이트)을 완료 절로 이동, 작업 성격 2건(DBGW 성능 개선·DBGWS 승인 절차)은 진행 중 유지, Takeaway에 '시점 지난 일정은 건강검진 때 내린다' 추가, updated 08-12→08-16. 재검증: 깨진 링크 0 / 고아 0 / 단방향 0 / Takeaway 위반 0 / frontmatter 결손 0 / 스키마 외 필드 0 / index 집합 일치. **미수정으로 남긴 것: Notion 재구축(35페이지 전량), 승격 후보 4건**. ④로 Notion 지연 총계는 안 변한다(mysql-operations는 이미 실질 지연 13에 포함)"

- [2026-08-16T20:05:00+09:00] LINT issues=4 broken_links=0 orphans=0 one_way_links=0 takeaway_violations=0 frontmatter=0 stale=0 contradictions=2 index_mismatch=0 raw_unprocessed=0 secrets=0 notion_rebuild_pending=1 promotion_candidates=5 schema_extra_fields=3 note="전 37페이지 3차 점검. **최우선 발견: Notion 재구축 미실행** — 앞 두 LINT가 `notion_lag=25`로 보고한 것의 실체다. 마지막 SYNC_NOTION이 08-15 23:20이고 08-16에는 0건인데, CLAUDE.md §6 '한 번뿐인 재구축'은 사용자가 Notion 페이지를 전부 삭제했음을 전제한다. 그런데 **32개 페이지가 아직 옛 `notion_page_id`를 보유** — 리셋 없이 동기화하면 §6이 경고한 대로 새로 만드는 대신 **휴지통 페이지를 되살린다**. 즉 지연 25건은 증상이고 원인은 재구축 1단계(`--reset-stamps`) 미실행이다. 지연 내역은 드리프트 없음(미동기화 3/실질 13/동일자 9). 신규 2건: **todo.md 방치**(날짜 항목 5개 전부 경과, 완료 절 공백, updated 나흘 정체 — 앞 두 LINT가 `개인` 카테고리를 점검 축에 안 넣어 놓쳤다) / **내부 불일치 2건**(operational-queries summary 11종 vs 10개 열거, index 미완 과제가 postgresql-object-operations 미동기화를 미추적). 자기 인벤토리 개수는 나머지 전부 일치(결함 5건×6곳·P1 6건·백로그 20건·오류 5건·XEvent 3종), 버전 하드 필터 수치도 전 페이지 일치. **오탐 2종 확정 — 다음 회차 재보고 불필요**: 백틱 안 문법 예시 링크 4건(위키가 obsidian-wiki-tooling-gotchas:35에 이미 문서화), 그리고 17:15 LINT가 도입한 `updated` vs git 날짜 대조 10건 — 저장소가 08-15 마이그레이션 커밋으로 시작해 **그 이전 `updated`를 가진 페이지에는 원리상 적용 불가**이며 Notion 스탬프 전용 커밋도 걸러야 한다. 이 점검은 08-15 이후 편집분에만 유효하다는 단서와 함께 index에 기록. 민감정보 0(재확인)"
