...
summary: 표준 장애 대응 5단계, 계층형 모니터링(SLO→클라우드→DB엔진→SQL), 고정 임계치 금지, AWS Database Insights 전환, 문서 생명주기 기준.
sources: ["Notion: 운영 및 모니터링 (2026-07-30)", "Notion: 정제된 핵심 가이드 (2026-07-30)"]
status: draft
created: 2026-08-04
updated: 2026-08-04
notion_page_id: "3befb969-b8be-8145-bf09-e2dd148ba397"
notion_synced: "2026-08-16T20:20:05+09:00"
---

> [!tip] 핵심 Takeaway
> - **임시 완화와 근본 조치를 분리하는 것이 5단계의 핵심.** 완화로 증상이 사라지면 근본 조치가 유실되므로, 장애 대응 에이전트도 이 둘을 별도 산출물로 내야 한다
> - **고정 임계치를 알람에 박지 않는다.** 인스턴스 크기·워크로드 기준선·SLO 기준으로 계산한다 — 임계치 자동 산출은 자동화할 가치가 가장 높은 지점
> - 분석 순서는 **SLO → 클라우드/호스트 → DB 엔진 → SQL.** 이 순서를 런북과 에이전트 프롬프트에 동일하게 박아 위에서부터 좁혀 들어가게 한다. 실행 절차는 [[monitoring-incident-runbook]]
> - **커뮤니케이션 단계가 빠져 있다** — 심각도 판정과 이해관계자 공지를 트랙으로 추가해야 실제 장애에서 쓸 수 있다 ^[inferred]
...
- [[monitoring-incident-runbook]] — 이 표준을 시간박스 절차와 임계치로 구체화한 실행 문서
- [[operational-queries]] — 3단계(대기 이벤트·잠금·실행 계획) 진단에 쓰는 쿼리
- [[dba-agent-work-plan]] — 이 절차를 에이전트로 옮기는 작업 계획
- [[notion-llm-wiki-governance]] — 문서 생명주기 기준의 상위 거버넌스
